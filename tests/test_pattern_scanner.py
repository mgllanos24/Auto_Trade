...
def test_calculate_rr_price_action_ignores_stale_structure(monkeypatch):
    highs = [90 + 0.5 * i for i in range(60)]
    lows = [88 + 0.5 * i for i in range(60)]

    highs[5] = 140.0
    lows[5] = 30.0

    df = _build_price_frame(highs, lows, [89 + 0.5 * i for i in range(60)])

    full_highs = list(highs)
    full_lows = list(lows)
    ...
    def fake_argrelextrema(values, _comparator, order=5):
        seq = list(values)
        if _matches(full_lows, seq):
            return (pattern_scanner.np.array([5]),)
        if _matches(full_highs, seq):
            return (pattern_scanner.np.array([5]),)
        return (pattern_scanner.np.array([]),)
    ...
    max_age = min(30, len(lows) - 1)
    low_window_start = max(0, len(lows) - 1 - max_age)
    recent_stop = min(lows[low_window_start:-1])
    recent_breakout = highs[-1]
    expected_target = recent_breakout + (recent_breakout - recent_stop)

    assert abs(rr_levels.stop - recent_stop) < 1e-9
    assert abs(rr_levels.breakout - recent_breakout) < 1e-9
    assert abs(rr_levels.target - expected_target) < 1e-9
    expected_rr = round((entry_price - recent_stop) / (expected_target - entry_price), 2)
    assert rr_levels.rr_ratio == expected_rr
