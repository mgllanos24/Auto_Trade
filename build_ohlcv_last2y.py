"""Build a 2-year OHLCV dataset for liquid US stocks.

This module centralises the only place we download market data. When the
existing ``us_liquid_stocks_ohlcv_last2y.csv`` already contains the latest
trading session, the script exits without contacting yfinance.  The resulting
CSV is used by the pattern scanners and screeners across the repository.
"""
