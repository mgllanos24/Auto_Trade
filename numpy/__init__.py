"""Compatibility shim for :mod:`numpy` used in the kata tests.

This repository ships with a tiny, pure Python implementation of the couple of
``numpy`` helpers that the unit tests exercise.  The lightweight version keeps
the exercises self contained and avoids the dependency on the real NumPy
package, which is not available in the execution environment used for the
automated tests.

On a developer's machine, however, the real NumPy package is usually installed
and third-party tools – for instance ``matplotlib`` – require the genuine
library.  Prior to this change Python would always resolve ``import numpy`` to
this stub package because it lives inside the project directory.  As a result
attempts to import packages that depend on the full NumPy distribution would
fail (``matplotlib`` expects ``numpy.exceptions.VisibleDeprecationWarning`` for
example).

To provide the best of both worlds we now attempt to import the real NumPy
module first.  If it is available we transparently expose it, otherwise we fall
back to the tiny in-repository implementation.
"""
...
def _load_real_numpy_if_available():
    """Return the genuine :mod:`numpy` module if it can be imported.

    We temporarily remove the project root from ``sys.path`` so that Python's
    import machinery does not resolve ``numpy`` to this very module.  When the
    import succeeds we update the current module's ``__dict__`` with the real
    implementation and register it in ``sys.modules``.  If the import fails we
    simply leave the stub implementation untouched.
    """

    project_root = Path(__file__).resolve().parent.parent
    original_sys_path = list(sys.path)
    existing_module = sys.modules.pop(__name__, None)
    try:
        sys.path = [p for p in sys.path if Path(p).resolve() != project_root]
        real_numpy = importlib.import_module("numpy")
    except ModuleNotFoundError:
        if existing_module is not None:
            sys.modules[__name__] = existing_module
        return None
    finally:
        sys.path = original_sys_path
        if __name__ not in sys.modules and existing_module is not None:
            sys.modules[__name__] = existing_module

    # Replace the current module contents with the real NumPy implementation.
    globals().update(real_numpy.__dict__)
    sys.modules[__name__] = real_numpy
    return real_numpy


_REAL_NUMPY = _load_real_numpy_if_available()
